...
create table if not exists public.solicitudes (
  id           bigint generated by default as identity primary key,
  kind         text not null,    -- voluntarios|agua|alimentos|insumos|insumos_medicos|transporte|refugio
...
create or replace function public.crear_solicitud(
  p_kind text, p_titulo text, p_detalle text, p_sector text,
...
begin
  if p_kind is null or p_kind not in
     ('voluntarios','agua','alimentos','insumos','insumos_medicos','transporte','refugio') then
  ...
  insert into public.solicitudes(kind,titulo,detalle,sector,contacto,organizacion,prioridad,estado,owner_token,lon,lat)
  values (p_kind, left(btrim(p_titulo),140), left(coalesce(p_detalle,''),1000),
...
grant execute on function public.crear_solicitud(text,text,text,text,text,text,text,text,double precision,double precision)
  to anon, authenticated;
